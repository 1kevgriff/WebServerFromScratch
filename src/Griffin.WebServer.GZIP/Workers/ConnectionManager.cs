...
public class ConnectionManagerWorker(IConfiguration configuration, ILogger<ConnectionManagerWorker> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            try
            {
                var request = await handler.GetMessageFromSocket();
                logger.LogInformation("[CLIENT]:{message}", request);

                var response = HttpResponse.Generate(request, _clientPath);
                await handler.SendResponse(response, request);
            }
            catch (Exception ex)
            {
